    ...
    <div className="flex min-h-screen flex-col bg-paper-50">
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10 sm:px-6">
        ...
        <form onSubmit={handleSubmit} noValidate className="mb-10 flex flex-col gap-5">
          ...
          <div>
            <Button type="submit" isLoading={isSubmitting} fullWidth={false} className="min-w-[180px]">
        ...
        <div>
          <h2 className="mb-4 text-lg font-semibold text-ink-900">Matérias cadastradas</h2>

          {isLoading && (
            <p className="inline-flex items-center gap-2 text-sm text-paper-500">
          ...
          {!isLoading && !loadError && materias.length === 0 && (
            <p className="text-sm text-ink-600">Nenhuma matéria cadastrada ainda.</p>
          )}
          ...
          {!isLoading && materias.length > 0 && (
            <ul className="flex flex-col gap-2">
              {materias.map((materia) => (
                ...
                >
                  <div>
                    <p className="text-sm font-medium text-ink-900">{materia.nome}</p>
                    {materia.area && <p className="text-xs text-ink-600">{materia.area}</p>}
                  </div>
                </li>
              ))}
            </ul>
        ...
        <Link
          to="/"
          className="mt-8 inline-block text-sm text-ink-600 transition-colors hover:text-ink-900"
        >
          Voltar ao início
        </Link>
      </main>

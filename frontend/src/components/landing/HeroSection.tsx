...
export function HeroSection() {
  return (
    <section className="grain border-b border-paper-200">
      <div className="relative z-1 mx-auto grid max-w-6xl gap-14 px-6 py-16 lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:gap-20 lg:py-24">
        <Reveal className="flex flex-col gap-6">
          ...
          <h1 className="max-w-xl text-4xl leading-[1.05] font-semibold tracking-tight text-balance text-ink-900 sm:text-[3.4rem]">
            Compare professores particulares antes de escolher.
          </h1>

          <p className="max-w-md leading-relaxed text-ink-600">
      ...
      <div className="relative z-1 border-t border-paper-200">
        <dl className="mx-auto grid max-w-6xl divide-y divide-paper-200 px-6 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          {FIGURES.map((figure) => (
            <div key={figure.label} className="py-7 sm:px-8 sm:first:pl-0 sm:last:pr-0">
              <dt className="tnum text-3xl font-semibold text-ink-900">
                <Counter to={figure.value} prefix={figure.prefix} />
              </dt>
              <dd className="mt-1 text-sm text-paper-600">{figure.label}</dd>
            </div>
          ))}
        </dl>
      </div>
